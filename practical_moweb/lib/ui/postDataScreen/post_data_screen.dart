...
class PostDataScreen extends StatefulWidget{
  var productName;
  var productId;
  PostDataScreen(this.productName,this.productId);
...
class _PostDataScreenState extends State<PostDataScreen> {
  ...
  void initState() {
      ...
      setState(() {
        switch(event.status){
          ...
          case Status.completed:
            ...
            var stringProductResponse = storage.read("productsFor${widget.productId}");
            allItemList = postDataResponseFromJson(stringProductResponse).response;
            isLoading =false;
            gotResponse = true;
            break;
          case Status.error:
    ...
    postDataBloc.postDataBlocMethod(PostDataRequest(category_id: widget.productId));
  }
        ...
        appBar: AppBar(
          ...
          title: Row(
            ...
            children: [
              ...
              Text(widget.productName,style: const TextStyle(color: Colors.black)),
  ...
  buildBodyWithList() {
    return ListView.builder(
      shrinkWrap: true,
      itemCount: searchKey.isNotEmpty?searchItemList.length:allItemList.length,
                    ...
                    child: Row(
                      children: [
                        FadeInImage.memoryNetwork(
                          ...
                          image: searchKey.isNotEmpty? baseUrl+ searchItemList[index].productImage[0]:baseUrl+allItemList[index].productImage[0],
                          fit: BoxFit.cover,
                        ),
                        Expanded(
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text("${searchKey.isNotEmpty? searchItemList[index].productName: allItemList[index].productName}",textAlign: TextAlign.start,
                                    style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 16)),
                                const SizedBox(height: 8),
                                Text("${searchKey.isNotEmpty? searchItemList[index].size:allItemList[index].size}"),
                                const SizedBox(height: 8),
                                Text("${searchKey.isNotEmpty?searchItemList[index].specialPrice<searchItemList[index].mainPrice?searchItemList[index].mainPrice:'':allItemList[index].specialPrice<allItemList[index].mainPrice?allItemList[index].mainPrice:''}",style: TextStyle(decoration: TextDecoration.lineThrough),),
                                SizedBox(
                                  width: MediaQuery.of(context).size.width,
                                  child: Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text("${searchKey.isNotEmpty?searchItemList[index].specialPrice:allItemList[index].specialPrice}",style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 18,color: Colors.deepOrangeAccent)),
                                            ...
                                            children: [
                                              IconButton(
                                                onPressed: (){
                                                  setState(() {
                                                    if(item==1){
                                                      allItemList[index].isAdded=false;
                                                    }else{
                                                      item--;
                                                    }
                                                  });
                                                },
                                                icon: const Icon(Icons.remove,size: 15, color: Colors.deepOrangeAccent)
                                              ),
                                              Text(item.toString(),style: const TextStyle(fontSize: 15,color: Colors.deepOrangeAccent,fontWeight: FontWeight.bold)),
                                              IconButton(
                                                onPressed: (){
                                                  setState(() {
                                                    item++;
                                                ...
                                                icon: const Icon(Icons.add,size: 15, color: Colors.deepOrangeAccent),
                                              )
                                            ],
                                          ),
                                        ),
                                      ...
                                      : Container(
                                          ...
                                          child: RaisedButton(
                                              ...
                                              onPressed: () {
                                                setState(() {
                                                  allItemList[index].isAdded = true;
                                                  print(index);
                                                });
                                              },
                                              color: Colors.white,
